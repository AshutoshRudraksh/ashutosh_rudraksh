...
import { Send, Terminal, User, Mail, CheckCircle, AlertCircle, Rocket, Volume2, VolumeX } from "lucide-react"
...
export default function TerminalContactFormEnhanced() {
  ...
  const typeMessage = async (message: string, type: TerminalLine["type"] = "output") => {
    ...
    if (type === "success") {
      ...
      await playError()
    } else if (message.includes("Processing") || message.includes("Initializing")) {
  ...
  const triggerRocketAnimation = async () => {
    ...
    // Play the rocket launch sequence
    await playRocketSequence()
  ...
  const handleCommand = async (command: string) => {
    ...
    switch (command.toLowerCase()) {
      ...
      case "help":
        ...
        await typeMessage("  audio    - Toggle sound effects", "output")
      ...
      case "about":
        ...
        await typeMessage("Features realistic rocket launch sound effects! 🚀", "output")
        await typeMessage("Designed to make contact forms less boring!", "output")
        break

      case "audio":
        toggleAudio()
        await typeMessage(`Sound effects ${audioEnabled ? "disabled" : "enabled"}`, "output")
        break
  ...
  const handleFormInput = async (input: string) => {
    switch (currentStep) {
      ...
      case "message":
        ...
        await typeMessage("🔥 Ignition systems online...", "output")
        await typeMessage("🌟 Initiating message deployment...", "output")
        setCurrentStep("sending")

        // Trigger rocket animation with sound
        await triggerRocketAnimation()
  ...
  return (
    <div className="terminal-container bg-[#0a0a0f] border border-[#00ff88]/30 rounded-lg overflow-hidden shadow-2xl shadow-[#00ff88]/10 relative">
      {/* Enhanced Rocket Launch Animation with Sound Visualization */}
      {showRocketAnimation && (
        <div className="rocket-animation-container">
          ...
          <div className="sound-waves">
            {Array.from({ length: 6 }).map((_, i) => (
              <div key={i} className="sound-wave" style={{ animationDelay: `${i * 0.3}s` }} />
            ))}
          </div>
        </div>
      )}

      {/* Terminal Header with Audio Controls */}
      <div className="terminal-header bg-[#1a1a2e] px-4 py-3 flex items-center justify-between border-b border-[#00ff88]/20">
        ...
        <div className="flex items-center space-x-4">
          <button
            onClick={toggleAudio}
            className="text-[#8892b0] hover:text-[#00ff88] transition-colors text-xs font-mono flex items-center space-x-1 ai-magical-glow"
      ...
      <div className="terminal-status bg-[#1a1a2e] px-4 py-2 border-t border-[#00ff88]/20 flex items-center justify-between text-xs font-mono">
        ...
        <div className="flex items-center space-x-2">
          <div className={`w-2 h-2 rounded-full ${audioEnabled ? "bg-[#00ff88] animate-pulse" : "bg-[#495670]"}`}></div>
          <span className="text-[#8892b0]">{audioEnabled ? "Audio On" : "Audio Off"}</span>
        </div>
      </div>
    </div>
  )
